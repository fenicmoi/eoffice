    ...
    <div class="col-md-10">
        <div class="panel panel-primary" style="margin: 10">
            ...
            <div class="panel-body">
                <form method="post">
                    <button type="submit" name="btnBackup" id="btnBackup" class="btn btn-warning pull-right"><i
                            class="fas fa-hdd"></i> Backup</button>
                    <button type="submit" name="btnRestore" id="btnRestore" class="btn btn-danger pull-right"><i
                            class="fas fa-history"></i> Restore</button>
                </form>
                        ...
                        public function backupTables($tables = '*')
                        {
                            try {
                                ...
                                foreach ($tables as $table) {
                                    ...
                                    $create_res = mysqli_query($this->conn, 'SHOW CREATE TABLE `' . $table . '`');
                                    if (!$create_res) {
                                        $this->obfPrint('ERROR fetching SHOW CREATE TABLE for ' . $table . ': ' . mysqli_error($this->conn));
                                        continue;
                                    }
                                    $row = mysqli_fetch_row($create_res);
                                    $sql .= "\n\n" . $row[1] . ";\n\n";
                                    ...
                                    $count_res = mysqli_query($this->conn, 'SELECT COUNT(*) FROM `' . $table . '`');
                                    if (!$count_res) {
                                        $this->obfPrint('ERROR fetching COUNT for ' . $table . ': ' . mysqli_error($this->conn));
                                        continue;
                                    }
