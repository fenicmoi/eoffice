...
<div class="row">
    ...
    <div class="col-md-10">
        <div class="panel panel-default" style="margin: 20">
            <div class="panel-heading"><i class="fa fa-address-book fa-2x" aria-hidden="true"></i>
                    ...
                    while ($row = dbFetchArray($result)) { ?>
                        <tr>
                            ...
                            <td>
                                <?php
                                if ($row['status'] == 1) { ?>
                                    <a class="btn btn-success" href="?open=<?php echo $row['rec_id']; ?>"
                                        onclick="return confirm('คุณกำลังจะปิดวาระ !'); ">อยู่ในวาระ</a>
                                <?php } else { ?>
                                    <a class="btn btn-danger" href="?close=<?php echo (int) $row['rec_id']; ?>"
                                        onclick="return confirm('คุณกำลังจะเปิดวาระ !'); ">หมดวาระ</a>
                                <?php } ?>
                            </td>
                            <td>
                                <a class="btn btn-danger" href="?del=<?php echo (int) $row['rec_id']; ?>"
                                    onclick="return confirm('กำลังจะลบข้อมูล !'); "><i class="fa fa-trash"
                            ...
                            <div class="form-group">
                                ...
                                <div class="input-group">
                                    <input type="text" class="form-control" id="name" name="name"
                                        placeholder="ไม่ต้องมีคำนำหน้า" required="">
                                    <span class="input-group-addon"><span class="glyphicon glyphicon-th-list"></span>
                                </div>
                            </div>
                            <div class="form-group">
...
if (isset($_POST['save'])) {
    ...
    $sec_id = $_POST['sec_id'];
    //check governer
    $sql = "SELECT rec_id FROM boss WHERE keyman = 1 AND status = 1";
    $result = dbQuery($sql);
    ...
    if ($row > 1) {
        ...
        $sql = "INSERT INTO boss(name, position, keyman, status, sec_id) VALUES (?, ?, ?, ?, ?)";
        $result = dbQuery($sql, "ssiii", [$name, $position, (int) $keyman, (int) $status, (int) $sec_id]);
...
if (isset($_GET['del'])) {
    $sql = "DELETE FROM boss WHERE rec_id = ?";
...
if (isset($_GET['close'])) {
    $sql = "UPDATE boss SET status = 1 WHERE rec_id = ?";
...
if (isset($_GET['open'])) {
    $sql = "UPDATE boss SET status = 0 WHERE rec_id = ?";
