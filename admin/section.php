            ...
            <table class="table table-bordered table-hover" id="myTable">
                ...
                <tbody>
                    ...
                    if ($u_id == 1) {     //ถ้าเป็น Admin  จะเห็นทั้งหมดเพื่อความสะดวกบริหารจัดการ
                        ...
                        $sql = "SELECT * FROM section WHERE dep_id = ?";    //กรณีเป็นบุคคลทั่วไปให้เห็นแค่เฉพาะในสังกัดของตนเอง
                    ...
                    while ($row = dbFetchArray($result)) { ?>
                        <tr>
                            ...
                            <td><?php echo htmlspecialchars($row['sec_code']); ?></td>
                            <td><?php
                            $status = $row['status'];
                            if ($status == 1) {
                            ...
                            <td><a class="btn btn-info" href="section_edit.php?edit=<?php echo $row['sec_id']; ?>"
                                    onclick="return confirm('กำลังจะแก้ไขข้อมูล !'); ">
                                    <i class="fas fa-edit" aria-hidden="true"></i> แก้ไข</a></td>
                            <td><a class="btn btn-danger" href="section.php?del=<?php echo $row['sec_id']; ?>"
                            ...
                            <div class="form-group">
                                ...
                                <div class="input-group">
                                    <input type="text" class="form-control" id="sec_name" name="sec_name"
                                        placeholder="ชื่อกลุ่ม/ฝ่าย" required>
                            ...
                            <div class="form-group">
                                ...
                                <div class="input-group">
                                    <input type="text" class="form-control" id="sec_code" name="sec_code"
                                        placeholder="หมายเลขประจำส่วนราชการ" required>
                                    <span class="input-group-addon"><span class="glyphicon glyphicon-th-list"></span>
                                </div>
...
if (isset($_POST['save'])) {
    ...
    $sql = "INSERT INTO section(sec_name, sec_code, dep_id, phone, fax, status, local_num)
                 VALUES(?, ?, ?, ?, ?, ?, ?)";
    $result = dbQuery($sql, "ssisssi", [$sec_name, $sec_code, (int) $dep_id, $phone, $fax, $status, (int) $local_num]);
    if (!$result) {
        echo "<script>
            swal({
...
if (isset($_GET['del'])) {
    $sql = "DELETE FROM section WHERE sec_id = ?";
