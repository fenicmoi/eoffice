...
$sql = "SELECT c.*, y.yname, u.firstname, d.dep_name 
      ...
      INNER JOIN depart as d ON d.dep_id = d.dep_id
      WHERE c.cid = ?
      ORDER BY c.cid DESC";
...
<form method="POST" name="upload" action="flow-command.php" enctype="multipart/form-data">
    ...
    <table class="table table-borderedr" border=1>
        ...
        <tr>
            ...
            <td><?php echo htmlspecialchars($row['yname']); ?></td>
        </tr>
        <tr>
            <td>เลขที่คำสั่ง</td>
            <td><?php echo htmlspecialchars($row['rec_id']); ?>/<?php echo htmlspecialchars($row['yname']); ?></td>
        </tr>
        <tr>
        ...
        <tr>
            <td>ผู้ลงนาม</td>
            <td><?php echo htmlspecialchars($row['boss']); ?></td>
        </tr>
        <tr>
            <td>คำสั่งเรื่อง</td>
        ...
        <tr>
            ...
            <?php } else { ?>
                <td>ไฟล์เอกสาร</td>
                <td> <a href="<?php print $row['file_upload']; ?>" class="btn btn-warning" target="_blank"><i
                            class="fas fa-download"></i>ดาวน์โหลด</a>
                <?php } ?>
