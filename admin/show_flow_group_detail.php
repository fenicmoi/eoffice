...
<form name="form_update" action="flow-resive-group.php" method="post">
    ...
    if ($numday > 3) { ?>
        <table border=1 width=100%>
            ...
            <tr>
                ...
                $sql = "SELECT u_id,firstname FROM user WHERE u_id = ?";
                $query = dbQuery($sql, "i", [$practice]);
    ...
    <?php } else { ?>
        <table class="table table-bordered" width=100%>
            ...
            <tr>
                <td><label>เลขหนังสือ:</label></td>
                <td><input type="text" name="book_no" id="book_no" class="form-control"
                        value="<?php echo $row['book_no']; ?>"></td>
            </tr>
            <tr>
                <td><label>เรื่อง:</label></td>
                <td><input type="text" name="title" id="title" class="form-control" value="<?php echo $row['title']; ?>">
                </td>
            </tr>
            <tr>
                <td><label>ผู้ส่ง:</label></td>
                <td><input type="text" name="sendfrom" id="sendfrom" class="form-control"
                        value="<?php echo $row['sendfrom']; ?>"></td>
            </tr>
            <tr>
                <td><label>ผู้รับ:</label></td>
                <td><input type="text" name="sendto" id="sendto" class="form-control" value="<?php echo $row['sendto']; ?>">
                </td>
            </tr>
            <tr>
                <td><label>ลงวันที่:</label></td>
                <td><input type="date" name="dateout" id="dateout" class="form-control"
                        value="<?php echo $row['dateout']; ?>"></td>
            </tr>
            <tr>
                <td><label>วันที่ลงรับ:</label></td>
                <td><input type="date" name="datein" id="datein" class="form-control" value="<?php echo $row['datein']; ?>">
                </td>
            </tr>
            <tr>
                <td><label>ผู้ปฏิบัติ:</label></td>
                ...
                $sec_id = $row['sec_id'];
                $p1 = $row['practice'];
                $sql = "SELECT u_id,sec_id,dep_id,firstname FROM user WHERE sec_id=?";
                $query = dbQuery($sql, "i", [$sec_id]);
                ?>
                    <?php $p1 = $row['practice']; ?>
