...
if ($strPage && is_numeric($strPage)) {
	$_current_page = intval($strPage);
	if ($_current_page == 0) {
		$_current_page = 1;
	}
}
...
function page_query($link, $sql, $rows_per_page = 10, $types = null, $params = null)
{
...
function page_link_font($size, $is_bold = false, $is_italic = false, $is_underline = true, $family = "inherit")
{
	...
	$_font_size = $size;
	if (_bool_type($is_bold)) {
		$_font_bold = "bold";
	}
	if (_bool_type($is_italic)) {
		$_font_italic = "italic";
	}
	if (!_bool_type($is_underline)) {
		$_text_decoration = "none";
	}
	$_font_family = $family;
}
