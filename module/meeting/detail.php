...
<body>
  ...
  <tr>
    ...
    <td><strong>
      <? 
	$sql="SELECT
			depart.name,user.name,user.telephone
		  FROM  depart
		    Inner Join user ON user.did = depart.did
		    Inner Join meeting_booking ON user.uid = meeting_booking.user_id
          WHERE user.uid = $user_id AND meeting_booking.book_id = $book_id";
		 // echo "$sql";
$dbquery=@mysql_db_query($dbname, $sql);
$result=@mysql_fetch_array($dbquery);
